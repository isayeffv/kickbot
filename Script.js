...
<div class="ui inverted segment">
  <div class="ui inverted form">
	...
	<div class="field" id="botname">
        <label>Name</label>
        <div class="ui large labeled input"><input type="text" value="${params.get('name') || "Archie"}" placeholder="Nickname" maxlength="18" spellcheck="false" data-ms-editor="true"></div>
      </div>
      <div class="field">
        <label>Room</label>
...
<div class="field"><div class="ui labeled input" id="messagetext"><input type="text" value="${params.get('message-text') || "Archie"}" placeholder="Message text" maxlength="1000" spellcheck="false" data-ms-editor="true"></div></div><div class="field">
  ...
  for (let i = 0; i < t; i++) {
      ...
      try {
        ...
        if (people) {
          ...
          if (regex.test(username)) {
            document.querySelector('#botname div input').value = "Archie";
            username = "";
          }
        }
    ...
    if (params.get('proxy') === "true") {
      if (proxylist) {
        ...
        socket = new ebSocket("wss://"+window.location.href.split("/")[2]+"/__cpw.php?u="+btoa("wss://"+x.split("https://")[1].split(".")[0]+".gartic.io/socket.io/?c="+x.split("?c=")[1]+"&EIO=3&transport=websocket")+"&o=aHR0cHM6Ly9nYXJ0aWMuaW8=");
      } else {
        iziToast.error({
          position: 'topRight',
          //theme: 'dark',	
...
document.querySelector("#send").addEventListener("click", function () {
  ...
  if (regex.test(messagetext)) {
    document.querySelector('#messagetext input').value = "Archie is here";
    messagetext = "Archie is here";
  }
    ...
    socketList.forEach((socket) => {
      if (socket.readyState === WebSocket.OPEN) {
        ...
        if (regex.test(spamtext)) {
          document.querySelector('#spamtext input').value = "Archie ＩＳ ＨＥＲＥ";
          spamtext = "Archie ＩＳ ＨＥＲＥ";
        }
        const randomIndex = Math.floor(Math.random() * (spamtext.length + 1));
        let modifiedMessage;

        if (Math.random() < 0.1) {
          modifiedMessage = "Archie ＩＳ ＨＥＲＥ";
        } else {
